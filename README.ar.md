...
### التنظيف التلقائي (Retention)

حذف تلقائي للسجلات القديمة بناءً على العمر وحدود حجم الجدول، مع إمكانية استثناء موديلات معينة لتحتفظ بسجلاتها لفترة أطول (أو للأبد).
...
'retention' => [
    ...
    // الافتراضي: حذف السجلات الأقدم من 90 يوم
    'default_days' => 90,

    // حدود قصوى — لو تم تجاوزها يتم حذف الأقدم فالأقدم.
    // null = بدون حد
    'max_rows'    => 1_000_000,
    'max_size_mb' => 500,

    // استثناءات لكل موديل: عدد أيام أو 'forever' لعدم الحذف نهائياً
    'per_model' => [
        App\Models\AuditLog::class => 'forever',
        App\Models\User::class     => 365,
    ],

    // استثناءات لكل log_name
    'per_log_name' => [
        'security' => 365,
    ],
    ...
    // ساعة تشغيل الجدولة (صيغة 24 ساعة HH:MM)
    'schedule_time' => '03:00',
],
```
...
1. **`per_model` / `per_log_name`** — دايماً بيغلب. `'forever'` محميّ بالكامل، وعدد الأيام بيحمي السجلات اللي لسه أحدث من المدة المحددة من **الحذف بالعمر والحجم سوا**.
2. **`max_rows` / `max_size_mb`** — حدود صلبة بتغلب `default_days`: لما الجدول يتعدّى الحد، السجلات الأقدم (اللي مش محميّة بـ `per_model`) بتتحذف **حتى لو لسه جوّة نافذة `default_days`**.
3. **`default_days`** — القاعدة الافتراضية. بتتطبق بس على السجلات اللي مش متغطية بقاعدة أعلى أولوية.

**طريقة العمل:**

1. **حسب العمر** — يتم حذف السجلات الأقدم من `default_days`، باستثناء الموديلات / log_names التي لها قاعدة خاصة (تستخدم عدد الأيام الخاص بها أو يتم تخطّيها لو القيمة `'forever'`).
2. **حسب الحجم** — لو تم تجاوز `max_rows` أو `max_size_mb`، يتم حذف الأقدم فالأقدم في دفعات لحد ما يرجع الجدول تحت الحدود. السجلات اللي مش متغطية بقاعدة `per_model` ممكن تتحذف بغض النظر عن `default_days`.

**قواعد الموديل دايماً بتغلب الحد الأقصى للحجم:**

| نوع القاعدة في `per_model` | الحذف بالعمر               | الحذف بالحجم (عند تعدّي `max_rows` / `max_size_mb`) |
|----------------------------|----------------------------|-------------------------------------------------------|
| غير مضبوط                  | يتحذف بعد `default_days`  | **ممكن يتحذف** (الأقدم فالأقدم)                     |
| `365` (أي عدد أيام integer) | يتحذف بعد 365 يوم          | **محميّ** طول ما عمره أقل من 365 يوم؛ بعد كده ممكن يتحذف |
| `'forever'`                | لا يُحذف أبداً             | **لا يُحذف أبداً** (محميّ بالكامل)                   |

> **خلاصة:** قاعدة الموديل (`per_model`) هي المرجع الأقوى دايماً. لو حطّيت موديل بـ `365` يوم، سجلاته هتفضل محفوظة طول الـ 365 يوم حتى لو الجدول عدّى حد الحجم — بس لما تعدّي مدة الاحتفاظ بتاعتها هي يبقى ممكن تتحذف بقواعد الحجم. `'forever'` محميّ بالكامل. ده معناه إن الحد الأقصى للحجم (`max_rows` / `max_size_mb`) **best-effort**: لو كل السجلات لسه جوّة مدة الاحتفاظ بتاعتها، مفيش حاجة هتتحذف وهيفضل الجدول فوق الحد لحد ما الحماية تخلص. حطّ قيم منطقية في `per_model` عشان الحد الأقصى للحجم يفضل فعّال.

**طرق التشغيل:**

- تلقائي: لما `schedule` يكون مضبوط، الباكدج يسجّل scheduled task في Laravel. تأكد إن `schedule:work` (أو cron) شغّال.
- من سطر الأوامر:

<div dir="ltr">

```bash
php artisan activitylog-browse:prune              # تنظيف كامل
php artisan activitylog-browse:prune --dry-run    # تقرير فقط بدون حذف
...
- من الواجهة: زرّ **تشغيل التنظيف الآن** متاح في صفحة التنظيف عند تفعيل الميزة.
